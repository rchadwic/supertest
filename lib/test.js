...
Test.prototype.end = function(fn) {
  ...
  var end = Request.prototype.end;
    ...
    if (server && server._handle) return server.close(assert);
                      ...
                      var data = "";
                      data += ("REQUEST SUPERTEST") +"\n"
                      data += ("_______________________________________") +"\n"
                      
                      data += (res.req._header)+"\n"
                      if(res.request._data){          
                        data += JSON.stringify(res.request._data)+"\n"
                        data += "\n";
                      }
                      
                      data += ("RESPONSE SUPERTEST")+"\n"
                      data += ("_______________________________________")+"\n"
                      data += ("HTTP/"+res.res.httpVersion +" "+ res.res.statusCode +" "+ res.res.statusMessage)+"\n"
                      for(var i in res.res.headers)
                      {
                          data += (i + ": " +res.res.headers[i])+"\n"
                      }
                      
                      if(res.res.body)
                      {
                        data += "\n";
                        data += JSON.stringify(res.res.body)+"\n";
                      }
